...
// mydecisive
func (n *SingleEndpointParser) PortsWithUrlPaths(logger logr.Logger, name string, config interface{}) ([]PortUrlPaths, error) {
	singleEndpointConfig := &SingleEndpointConfig{}
	if err := mapstructure.Decode(config, singleEndpointConfig); err != nil {
		return []PortUrlPaths{}, nil
	}
	if _, err := singleEndpointConfig.GetPortNum(); err != nil && n.svcPort.Port == UnsetPort {
		if n.failSilently {
			logger.WithValues("receiver", n.name).V(4).Info("couldn't parse the endpoint's port and no default port set", "error", err)
			err = nil
		} else {
			logger.WithValues("receiver", n.name).Error(err, "couldn't parse the endpoint's port and no default port set")
		}
		return []PortUrlPaths{}, err
	}

	port := singleEndpointConfig.GetPortNumOrDefault(logger, n.svcPort.Port)
	n.svcPort.Name = naming.PortName(name, port)
	servicePort := ConstructServicePort(n.svcPort, port)
	return []PortUrlPaths{{servicePort, n.urlPaths}}, nil
}
...
func NewSinglePortParser(name string, port int32, opts ...PortBuilderOption) *SingleEndpointParser {
	...
	return &SingleEndpointParser{name: name, svcPort: &servicePortWithUrlPaths.Port}
}
...
func NewSilentSinglePortParser(name string, port int32, opts ...PortBuilderOption) *SingleEndpointParser {
	...
	return &SingleEndpointParser{name: name, svcPort: &servicePortWithUrlPaths.Port, failSilently: true}
}
